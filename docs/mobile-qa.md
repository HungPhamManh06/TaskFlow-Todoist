...
> Simulated in Playwright (chromium, headless) at 2026-08-11 14:40 — **no physical device**. Software keyboard and real notch safe-area insets (`env(safe-area-inset-*)` = 0 headless) are not covered; everything else is real layout/geometry from the actual app.
...
| 360x800 (small Android) | auth | modal fits viewport (no cut-off) | PASS | (h=424px) |
...
| 390x844 (common mobile) | auth | modal fits viewport (no cut-off) | PASS | (h=404px) |
...
| 412x915 (large mobile) | auth | modal fits viewport (no cut-off) | PASS | (h=404px) |
...
| 768x1024 (tablet) | quick add | task created (count +1) | PASS | (5 -> 6) |
...
| 768x1024 (tablet) | auth | modal fits viewport (no cut-off) | PASS | (h=404px) |

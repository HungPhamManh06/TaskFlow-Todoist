...
Lỗi được chuẩn hoá (không lộ lỗi upstream thô): `ai-not-configured` · `ai-timeout` · `ai-rate-limited` · `ai-provider-bad-request` (upstream 400) · `ai-provider-auth` (401) · `ai-provider-forbidden` (403) · `ai-provider-not-found` (404) · `ai-provider-unavailable` (5xx/network) · `ai-invalid-response` (+ `details: ["parse-failed" | "empty-content"]`) · `ai-validation-failed` (+ `details: ["action-0-unknown-task", …]`). Mọi lỗi upstream HTTP kèm `details: ["upstream-<status>"]` — không bao giờ body upstream. Mọi lỗi đều để client fallback về planner quy tắc.

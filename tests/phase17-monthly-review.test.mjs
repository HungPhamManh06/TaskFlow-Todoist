...
test('Monthly Review production assets load before report/app and cache offline', () => {
  ...
  const reportIndex = APP_HTML.indexOf('js/report-ui.min.js?v=4');
  const appIndex = APP_HTML.indexOf('js/app.min.js?v=174');
  assert.ok(monthlyIndex >= 0 && reportIndex > monthlyIndex && appIndex > reportIndex);
  assert.match(APP_HTML, /js\/i18n\.min\.js\?v=15/);

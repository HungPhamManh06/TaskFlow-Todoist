...
test('Weekly Review production asset loads before app and is cached offline', () => {
  ...
  const appIndex = APP_HTML.indexOf('js/app.min.js?v=167');
  assert.ok(reviewIndex >= 0);
  assert.ok(appIndex > reviewIndex);
  assert.match(APP_HTML, /js\/i18n\.min\.js\?v=9/);
  assert.equal((APP_HTML.match(/css\/styles-deferred\.min\.css\?v=12/g) || []).length, 2);
